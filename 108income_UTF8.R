...
ins.pack("tidyverse", "docxtractr", "readtext", 
         "haven", "hablar", "readxl", "magrittr", 
         "data.table"); rm(ins.pack)
# options
...
ptm <- proc.time()
# data source
path_code <- "108/code107.docx"
path_dat <- "108/HDATA108"
year <- 108
...
varify <- function(data = df.inc108, weight, item) {
        df <- data
        ...
        # weight table
        wtab <- round(xtabs(df[[weight]] ~ df[[item]]))
        # replicate income by weight
        i <- names(wtab)
        weighed <- mapply(rep, x = i, times = wtab)
        weighed <- as.numeric(unlist(weighed, use.names = TRUE))
        # return(median(weighed, na.rm = TRUE))
        return(sum(weighed, na.rm = TRUE))
}

# verify ------------------------------------------------------------------


xl <- read_xlsx(path = "108/108年各科目金額表.xlsx", range = "B3:C48")
xl[27, 1] <- "rent"
xl[28, 1] <- "electricity"
xl[ , 1] <- paste0("itm", xl$代號)

# setDT
setDT(df.inc108)

# number of people in the household
lb1 <- grep("^b1_", names(df.inc108))
lb10 <- grep("^itm[1][0][4][1-6]", names(df.inc108))
lb1079 <- grep("^itm[1][0][4][7-9]", names(df.inc108))

# recode: n.all
df.inc108[ , n.all := rowSums(!(is.na(.SD)), na.rm = TRUE), .SDcols = lb1]
# recode: itmrent
df.inc108[ , itmrent := rowSums(.SD, na.rm = TRUE), .SDcols = lb10]
# recode: itmelectricity
df.inc108[ , itmelectricity := rowSums(.SD, na.rm = TRUE), .SDcols = lb1079]
# recode
df.inc108 %<>% mutate(itm400 = if_else(is.na(itm400), 0, itm400), 
                      itm600 = if_else(is.na(itm600), 0, itm600), 
                      # disposable income
                      disp_inc = itm400 - itm600, 
                      # eq dis income
                      eq_inc = disp_inc / sqrt(n.all), 
                      # average income
                      itminc = disp_inc / n.all, 
                      # rent
                      # itmrent = itm1041 + itm1042 + itm1043 + itm1044 + itm1045 + itm1046, 
                      # electricity
                      # itmelectricity = itm1047 + itm1048 + itm1049
                      )
setDT(xl)
xl[ , varify := mapply(varify, weight = "a20", item = xl$代號)]
write_excel_csv(xl, "108/varify.csv")

# save --------------------------------------------------------------------
# .RData
# save df.inc
save(df.inc108, file = "108/df_inc108.RData")
save(df.inc108, file = "R data files/df_inc108.RData")
saveRDS(df.inc108, file = "R data files/df_inc108.rds")

# save code_tbl
save(code_tbl_108, file = "108/code_tbl_108.RData")
save(code_tbl_108, file = "R data files/code_tbl_108.RData")
saveRDS(code_tbl_108, file = "R data files/code_tbl_108.rds")
# .csv format
