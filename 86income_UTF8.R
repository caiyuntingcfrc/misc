##### author: CAI YUN-TING ######
##### The Survey of Family Income and Expenditure, 1997 #####
...
# data source
path_code <- "AA170022/code86.docx"
path_dat <- "AA170022/inc86.dat"
year <- 86

# spss file for ref
# d.spss <- read_sav("AA170022/inc86.sav")
...
doc.items <- grep("：", doc.text.parts, value = TRUE) %>% 
        # begin with three digits and character "："
        grep("^[1-9][0-9][0-9]：", ., value = TRUE)
# item numbers
doc.items.digits <- strsplit(doc.items, "：") %>% 
        unlist() %>% 
        .[2 * (1:length(doc.text.parts)) -1 ] %>% 
        .[!is.na(.)]
# item contents
doc.items.contents <- strsplit(doc.items, "：") %>% 
        unlist() %>% 
...
l <- grep("x1|b4|b17|b18|b23|b25", variables)
# mutate_if
...
write(x, file = y)
# code_tbl[86:112]
l <- grep("x1|^c[0-9]", code_tbl$variable)
...
for(i in 0:4) {
        x[[i + 1]] <- read_fwf(y, fwf_positions(c(1, 9 + i * 14, 12 + i * 14),
                                            c(8, 11 + i * 14, 22 + i * 14),
...
# .RData
# save df.inc (RData)
save(df.inc86, file = "AA170022/df_inc86.RData")
save(df.inc86, file = "R data files/df_inc86.RData")
# save df.inc (rds)
saveRDS(df.inc86, file = "AA170022/df_inc86.rds")
saveRDS(df.inc86, file = "R data files/df_inc86.rds")
# save code_tbl
save(code_tbl_86, file = "AA170022/code_tbl_86.RData")
save(code_tbl_86, file = "R data files/code_tbl_86.RData")
# save code_tbl (rds)
saveRDS(code_tbl_86, file = "AA170022/code_tbl_86.rds")
saveRDS(code_tbl_86, file = "R data files/code_tbl_86.rds")
