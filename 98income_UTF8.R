##### author: CAI YUN-TING ######
##### The Survey of Family Income and Expenditure, 2009  #####
...
# data source
path_code <- "AA170034/code98.docx"
path_dat <- "AA170034/inc98.dat"
year <- 98
...
doc.text.parts <- readtext(path_code)$text %>% 
        strsplit("\n") %>% .[[1]]
# items
doc.items <- grep("\\s{2}", doc.text.parts, value = TRUE) %>% 
        # begin with three digits and character "  "
        grep("^itm[1-9][0-9]", ., value = TRUE) %>% 
        strsplit(., "itm") %>% 
        unlist() %>% 
        .[!(. == "")] %>% 
        strsplit(., "\\s{2}") %>% 
...
# .RData
# save df.inc
save(df.inc98, file = "AA170034/df_inc98.RData")
save(df.inc98, file = "R data files/df_inc98.RData")
# save code_tbl
save(code_tbl_98, file = "AA170034/code_tbl_98.RData")
save(code_tbl_98, file = "R data files/code_tbl_98.RData")
# .csv format
